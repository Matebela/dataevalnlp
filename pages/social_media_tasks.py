    ...
    if st.button("Generate Social Media Posts"):
        if context and topic:
            ...
            if social_media_content:
                ...
                for i, post in enumerate(social_media_content['LinkedIn'], start=1):
                    st.text_area(f"LinkedIn Post {i}", post, height=150)
                for i, post in enumerate(social_media_content['Twitter'], start=1):
                    st.text_area(f"Twitter Post {i}", post, height=100)
            else:
...
def generate_social_media_posts(context, topic):
    ...
    * Leverage data analytics for strategic insights.
    ...
    try:
        ...
        # Split the content into LinkedIn and Twitter posts
        linkedin_posts = []
        twitter_posts = []
        lines = social_media_content.split('\n')
        for line in lines:
            if line.startswith("LinkedIn Post"):
                linkedin_posts.append(line.split(': ', 1)[1])
            elif line.startswith("Twitter Post"):
                twitter_posts.append(line.split(': ', 1)[1])
